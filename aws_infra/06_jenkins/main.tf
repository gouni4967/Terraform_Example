resource "aws_instance" "aws06_jenkins_server" {
  ami           = var.ami_id
  ...
  subnet_id = data.aws_subnets.aws06_private_subnets.ids[0]
  ...
  iam_instance_profile = data.aws_iam_instance_profile.aws06_ec2_profile.name
...
resource "aws_lb_target_group_attachment" "aws06_jenkins_attach" {
  target_group_arn = data.aws_lb_target_group.aws06_jenkins_tg.arn
  target_id        = aws_instance.aws06_jenkins_server.id
  port             = 80
}